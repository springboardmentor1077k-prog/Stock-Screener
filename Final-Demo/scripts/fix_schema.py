...
SYMBOL = "PFE"  # Change this to fetch other stocks
DATABASE_CONFIG = {
...
def fetch_full_stock_data():
    ...
    if existing_stock:
        ...
        print(f"⚠️ Found existing {SYMBOL} with ID {stock_id}. Updating...")
        cur.execute("""
            UPDATE stocks 
    ...
    # --- UPSERT FUNDAMENTALS (Fixed to include Dividend & Debt) ---
    ...
    # FIX: Added dividend_yield and debt_to_equity to the query
    cur.execute("""
    ...
    """, (
        ...
        parse_float(data_ov.get("DividendYield")),  # Added DividendYield
        parse_float(data_ov.get("RevenueTTM")),     # AlphaVantage Overview typically lacks DebtToEquity, check your API response or leave as NULL
        updated_at_date
    ))
    ...
    # ======================================================
    # PART 2: FETCH QUARTERLY HISTORY
    # ======================================================
    ...
    if not quarterly_reports:
        print("⚠️ No quarterly reports found.")
    else:
        count = 0
