...
class AIBackend:
    ...
    def _generate_sql(self, query):
        ...
        for sector in sectors:
            if sector in query:
                # Capitalize correctly for DB match (simple case)
                db_sector = sector.upper() if len(sector) <= 3 else sector.title()
                if sector == 'it': db_sector = 'IT' # Special case
    ...
    def _extract_number(self, query, context_phrase):
        ...
        This is a heuristic.
        """
        # Look for digits around the context phrase
        # Split query and find the token after the metric and operator
        # This is tricky with regex, let's try a simple search for digits
        # A robust way for this demo: look for digits in the string. 
        # If multiple metrics, this might fail, but for the demo queries it should work.
        
        # Better approach: Regex specific to the metric context
        # e.g. "pe ratio less than 20" -> search for "20" after "pe ratio"
