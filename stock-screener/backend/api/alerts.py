from fastapi import APIRouter, Depends, HTTPException, status
from pydantic import BaseModel
from typing import List, Optional
from backend.api.auth import get_current_user
from backend.api.database import get_db
import psycopg2
from psycopg2 import extras
from datetime import datetime

router = APIRouter(prefix="/alerts", tags=["alerts"])

class AlertCreate(BaseModel):
    stock_id: int
    portfolio_id: int
    metric: str  
    operator: str 
    threshold: float

class AlertResponse(BaseModel):
    alert_id: int
    user_id: int
    stock_id: int
    portfolio_id: int
    symbol: str
    company_name: str
    metric: str
    operator: str
    threshold: float
    is_active: bool
    created_at: datetime
    times_triggered: int
    last_triggered: Optional[datetime]

class AlertEventResponse(BaseModel):
    event_id: int
    alert_id: int
    stock_id: int
    symbol: str
    company_name: str
    metric: str
    operator: str
    threshold: float
    triggered_value: float
    triggered_at: datetime

@router.get("/", response_model=List[AlertResponse])
async def get_user_alerts(current_user: dict = Depends(get_current_user)):
    """Get all alerts for the current user."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        db_cursor.execute("""
            SELECT 
                a.alert_id,
                a.user_id,
                a.stock_id,
                a.portfolio_id,
                s.symbol,
                s.company_name,
                a.metric,
                a.operator,
                a.threshold,
                a.is_active,
                a.created_at,
                COUNT(ae.event_id) as times_triggered,
                MAX(ae.triggered_at) as last_triggered
            FROM alerts a
            JOIN stocks s ON a.stock_id = s.stock_id
            LEFT JOIN alert_event ae ON a.alert_id = ae.alert_id
            WHERE a.user_id = %s
            GROUP BY a.alert_id, a.user_id, a.stock_id, a.portfolio_id, s.symbol, s.company_name, 
                     a.metric, a.operator, a.threshold, a.is_active, a.created_at
            ORDER BY a.created_at DESC
        """, (current_user["user_id"],))
        
        alerts_list = db_cursor.fetchall()
        return alerts_list
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.post("/", response_model=dict)
async def create_alert(alert_request: AlertCreate, current_user: dict = Depends(get_current_user)):
    """Create a new alert for the current user."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        valid_operators = ['>', '<', '>=', '<=', '=']
        if alert_request.operator not in valid_operators:
            raise HTTPException(status_code=400, detail=f"Invalid operator. Must be one of: {', '.join(valid_operators)}")
        
        valid_metrics = ['price', 'pe_ratio', 'market_cap', 'eps', 'roe', 'dividend_yield']
        if alert_request.metric not in valid_metrics:
            raise HTTPException(status_code=400, detail=f"Invalid metric. Must be one of: {', '.join(valid_metrics)}")
        
        db_cursor.execute("SELECT stock_id FROM stocks WHERE stock_id = %s", (alert_request.stock_id,))
        if not db_cursor.fetchone():
            raise HTTPException(status_code=404, detail="Stock not found")
        
        db_cursor.execute("SELECT portfolio_id FROM portfolio WHERE portfolio_id = %s AND user_id = %s", 
                      (alert_request.portfolio_id, current_user["user_id"]))
        if not db_cursor.fetchone():
            raise HTTPException(status_code=404, detail="Portfolio not found or doesn't belong to you")
        
        db_cursor.execute("""
            INSERT INTO alerts (user_id, stock_id, portfolio_id, metric, operator, threshold, is_active)
            VALUES (%s, %s, %s, %s, %s, %s, TRUE)
            RETURNING alert_id
        """, (current_user["user_id"], alert_request.stock_id, alert_request.portfolio_id, alert_request.metric, alert_request.operator, alert_request.threshold))
        
        row = db_cursor.fetchone()
        db_connection.commit()
        
        return {"alert_id": row['alert_id'], "message": "Alert created successfully"}
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.delete("/{alert_id}")
async def delete_alert(alert_id: int, current_user: dict = Depends(get_current_user)):
    """Delete an alert."""
    db_connection = get_db()
    db_cursor = db_connection.cursor()
    
    try:
        db_cursor.execute("""
            SELECT alert_id FROM alerts 
            WHERE alert_id = %s AND user_id = %s
        """, (alert_id, current_user["user_id"]))
        
        if not db_cursor.fetchone():
            raise HTTPException(status_code=404, detail="Alert not found")
        
        db_cursor.execute("DELETE FROM alerts WHERE alert_id = %s", (alert_id,))
        db_connection.commit()
        
        return {"message": "Alert deleted successfully"}
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.patch("/{alert_id}/toggle")
async def toggle_alert(alert_id: int, current_user: dict = Depends(get_current_user)):
    """Toggle alert active status."""
    db_connection = get_db()
    db_cursor = db_connection.cursor()
    
    try:
        db_cursor.execute("""
            SELECT is_active FROM alerts 
            WHERE alert_id = %s AND user_id = %s
        """, (alert_id, current_user["user_id"]))
        
        result = db_cursor.fetchone()
        if not result:
            raise HTTPException(status_code=404, detail="Alert not found")
        
        new_status = not result[0]
        db_cursor.execute("""
            UPDATE alerts SET is_active = %s 
            WHERE alert_id = %s
        """, (new_status, alert_id))
        db_connection.commit()
        
        return {"message": f"Alert {'activated' if new_status else 'deactivated'} successfully", "is_active": new_status}
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.get("/events", response_model=List[AlertEventResponse])
async def get_alert_events(current_user: dict = Depends(get_current_user), limit: int = 50):
    """Get most recent alert event for each alert."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        db_cursor.execute("""
            SELECT 
                ae.event_id,
                ae.alert_id,
                ae.stock_id,
                s.symbol,
                s.company_name,
                a.metric,
                a.operator,
                a.threshold,
                ae.triggered_value,
                ae.triggered_at
            FROM alert_event ae
            JOIN alerts a ON ae.alert_id = a.alert_id
            JOIN stocks s ON ae.stock_id = s.stock_id
            WHERE a.user_id = %s
            AND ae.triggered_at = (
                SELECT MAX(ae2.triggered_at)
                FROM alert_event ae2
                WHERE ae2.alert_id = ae.alert_id
            )
            ORDER BY ae.triggered_at DESC
            LIMIT %s
        """, (current_user["user_id"], limit))
        
        events = db_cursor.fetchall()
        return events
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.get("/stocks/search")
async def search_stock_by_symbol(symbol: str):
    """Search for a stock by symbol."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        db_cursor.execute("""
            SELECT stock_id, symbol, company_name 
            FROM stocks 
            WHERE symbol = %s
        """, (symbol.upper(),))
        
        stock = db_cursor.fetchone()
        if stock:
            return stock
        else:
            raise HTTPException(status_code=404, detail=f"Stock symbol '{symbol}' not found")
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.get("/stocks/list")
async def list_all_stocks():
    """Get all stocks for dropdown selection."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        db_cursor.execute("""
            SELECT stock_id, symbol, company_name 
            FROM stocks 
            ORDER BY symbol
        """)
        
        stocks_list = db_cursor.fetchall()
        return stocks_list
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.get("/summary")
async def get_alert_summary(current_user: dict = Depends(get_current_user)):
    """Get alert summary for the current user."""
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        db_cursor.execute("""
            SELECT 
                COUNT(DISTINCT a.alert_id) as total_alerts,
                SUM(CASE WHEN a.is_active = TRUE THEN 1 ELSE 0 END) as active_alerts,
                COUNT(DISTINCT ae.event_id) as total_triggers,
                COUNT(DISTINCT a.stock_id) as stocks_monitored
            FROM alerts a
            LEFT JOIN alert_event ae ON a.alert_id = ae.alert_id
            WHERE a.user_id = %s
        """, (current_user["user_id"],))
        
        summary = db_cursor.fetchone()
        return summary
        
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()

@router.get("/poll")
async def poll_alerts(current_user: dict = Depends(get_current_user)):
    """
    Automated check for active alerts. 
    Can be called periodically by the frontend.
    """
    db_connection = get_db()
    db_cursor = db_connection.cursor(cursor_factory=extras.RealDictCursor)
    
    try:
        # Get active alerts that HAVEN'T been triggered in the last 15 minutes
        db_cursor.execute("""
            SELECT a.alert_id, a.stock_id, s.symbol, a.metric, a.operator, a.threshold, 
                   f.current_price as price, f.pe_ratio, f.market_cap, f.roe, f.dividend_yield, f.eps
            FROM alerts a
            JOIN stocks s ON a.stock_id = s.stock_id
            JOIN fundamentals f ON a.stock_id = f.stock_id
            WHERE a.user_id = %s 
              AND a.is_active = TRUE
              AND a.alert_id NOT IN (
                  SELECT alert_id 
                  FROM alert_event 
                  WHERE triggered_at > NOW() - INTERVAL '15 minutes'
              )
        """, (current_user["user_id"],))
        
        active_alerts = db_cursor.fetchall()
        triggered_details = []
        
        for alert in active_alerts:
            val = alert.get(alert['metric'])
            if val is None: continue
            
            triggered = False
            op = alert['operator']
            threshold = alert['threshold']
            
            if op == '>' and val > threshold: triggered = True
            elif op == '<' and val < threshold: triggered = True
            elif op == '>=' and val >= threshold: triggered = True
            elif op == '<=' and val <= threshold: triggered = True
            elif op == '=' and val == threshold: triggered = True
            
            if triggered:
                # Log the event
                db_cursor.execute("""
                    INSERT INTO alert_event (alert_id, stock_id, triggered_value)
                    VALUES (%s, %s, %s)
                """, (alert['alert_id'], alert['stock_id'], val))
                
                triggered_details.append({
                    "symbol": alert['symbol'],
                    "metric": alert['metric'].replace('_', ' ').title(),
                    "operator": op,
                    "threshold": threshold,
                    "value": val
                })
        
        db_connection.commit()
        return {"triggered_alerts": triggered_details}
        
    except psycopg2.Error as e:
        db_connection.rollback()
        raise HTTPException(status_code=500, detail=f"Polling error: {str(e)}")
    finally:
        db_cursor.close()
        db_connection.close()
